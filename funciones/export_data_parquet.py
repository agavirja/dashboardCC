...
def getdata():
    ...
    dfmerge    = datacatastro.groupby('chip').agg({'avaluocatastral':'max','estrato':'max'}).reset_index()
    df1        = df1.merge(dfmerge,on='chip',how='left',validate='m:1')
    df1        = df1.sort_values(by=['numero','chip','year'],ascending=False).drop_duplicates(subset=['numero','chip'],keep='first')
    df         = df1.groupby('numero').agg({'avaluocatastral':'sum','estrato':'max'}).reset_index()
    df.columns = ['numID','avaluocatastral','estrato']
    data       = data.merge(df,on='numID',how='left',validate='m:1')
    ...
    schema   = os.getenv("schema_bigdata")
    engine   = create_engine(f'mysql+mysqlconnector://{user}:{password}@{host}/{schema}')
    datalocalidad = pd.read_sql_query("SELECT locnombre, loccodigo, ST_AsText(geometry) as wkt FROM  bigdata.data_bogota_localidades" , engine)
    ...
    datacatastro = gpd.GeoDataFrame(datacatastro,geometry=gpd.points_from_xy(datacatastro['longitud'], datacatastro['latitud']),crs="EPSG:4326")
    datacatastro = gpd.sjoin(datacatastro,datalocalidad[['loccodigo','locnombre','geometry']],how='inner',predicate='within')
    
    
    datamerge    = datacatastro.drop_duplicates(subset='chip',keep='first')
    datamerge.rename(columns={'prenbarrio':'scanombre'},inplace=True)
    df1          = df1.merge(datamerge[['chip','loccodigo','locnombre','scacodigo', 'scanombre']],on=['chip'],how='left',validate='m:1')
    df           = df1.groupby('numero').agg( chips=('chip', lambda x: list(x.unique())) , locn=('loccodigo', lambda x: list(x.unique())) , locc=('locnombre', lambda x: list(x.unique())) , scac=('scacodigo', lambda x: list(x.unique())) , scan=('scanombre', lambda x: list(x.unique()))).reset_index()
    df.columns = ['numID','chip','loccodigo','locnombre','scacodigo', 'scanombre']
    data       = data.merge(df,on='numID',how='left',validate='m:1')

    del datalocalidad['geometry']
    del databarrios['geometry']
