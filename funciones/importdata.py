...
def getdata():
    ...
    #dataplacas = pd.read_excel('data/PlacasFontanar.xlsx')
    dataplacas.columns = [x.lower() for x in list(dataplacas)]
    dataplacas.rename(columns={'placas':'placa'},inplace=True)
    labels.append({'value':len(dataplacas['placa'].unique()),'name':'placas','label':'Total placas'})
    
    #dataplacas = pd.concat([pd.DataFrame([{'placa':'DQL490'}]),dataplacas])
    ...
    labels.append({'value':len(data['placa'].unique()),'name':'placas_match','label':'Placas con información del vehículo'})
    ...
    labels.append({'value':len(datamerge['numID'].unique()),'name':'contactos_match','label':'Información de contacto'})
    ...
    labels.append({'value':len(dataid['numero'].unique()),'name':'propiedades_match','label':'Información de propiedades'})
    ...
    return labels,data,databarrios,datalocalidad
